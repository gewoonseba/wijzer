...
function ProvenanceLabel({ source }: { source: string }) {
  ...
  return (
    <span className="ml-1 text-xs text-[var(--muted)]">({source})</span>
  );
}

export function ClipMetadataPanel({ metadata }: { metadata: ClipMetadata }) {
  return (
    <dl className="grid gap-2 rounded-lg border border-[var(--border)] p-4 text-sm">
      <div className="flex gap-2">
        <dt className="text-[var(--muted)]">Kind</dt>
        <dd className="capitalize">{metadata.kind}</dd>
      </div>
      <div className="flex gap-2">
        <dt className="text-[var(--muted)]">Tool</dt>
      ...
      {metadata.warnings.length > 0 ? (
        <div>
          <dt className="mb-1 text-[var(--muted)]">Warnings</dt>
          <dd>
